...
export default function DifficultAirwayScreen() {
  ...
  const flatListRef = useRef<FlatList<DASPlan>>(null);

  const [selectedPlan, setSelectedPlan] = useState<PlanID>("A");

  const [checkedSteps, setCheckedSteps] = useState<Record<PlanID, string[]>>({
    A: [],
    B: [],
    C: [],
    D: [],
  ...
  // Performs toggling of the checklist
  ...
  // Allows for movement from one plan to the other
  ...
  // For a smooth and seemless transition to the next plan
  // ChatGPT helped generate this whole line of code
  const handleSwipe = (event: NativeSyntheticEvent<NativeScrollEvent>) => {
    const index = Math.round(event.nativeEvent.contentOffset.x / width);

    const plans: PlanID[] = ["A", "B", "C", "D"];
  ...
  // handles the movement to the next plan
  const handleNext = () => {
    const current = dasPlans.find((p) => p.id === selectedPlan);

    if (current?.nextPlan) {
      changePlan(current.nextPlan as PlanID);
    }
  };

  // Resets all checklists for each plan when user resets the plan
  const handleReset = () => {
    setCheckedSteps({
      A: [],
      B: [],
    ...
    <SafeAreaView style={styles.container} edges={["top"]}>
      ...
      >
        <View style={styles.headerContainer}>
          ...
          <DASTabs selected={selectedPlan} onSelect={changePlan} />
        </View>
        ...
        <FlatList
          ...
          renderItem={({ item }) => {
            const planId = item.id as PlanID;
            ...
            return (
              ...
              >
                <DASCard
                  plan={item}
                  checkedSteps={checkedSteps[planId]}
